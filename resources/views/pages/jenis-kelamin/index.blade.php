...
@section('nav-master-jenis-kelamin', 'open active')

@section('content')
   <div class="d-flex justify-content-between align-items-center mb-4">
      <h4 class="fw-bold mb-0">DATA JABATAN</h4>
      <div>
         <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modal-create">Tambah Data</button>
      </div>
   </div>
   <div class="card">
                  ...
                  <tr>
                     ...
                     <td class="fit-center">
                        <button class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#modal-edit"
                           onclick="handleEdit({{ $item }},'{{ route('jenis-kelamin.update', $item) }}')">Edit</button>
                        <button class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#modal-delete"
                           onclick="handleDelete({{ $item }},'{{ route('jenis-kelamin.destroy', $item) }}')">Delete</button>
                     </td>
                  </tr>
               @endforeach
            </tbody>
      ...
      <div class="modal-dialog modal-dialog-centered">
         <div class="modal-content">
            ...
            <div class="modal-body">
               <form id="form-create" method="POST" action="{{ route('jenis-kelamin.store') }}">
                  @csrf
                  <div class="mb-3">
...
@push('scripts')
   <script>
      function handleEdit(jabatan, url) {
         $('#form-edit').attr('action', url);
         $('#nama-edit').val(jabatan.nama);
      }

      function handleDelete(jabatan, url) {
         $('#form-delete').attr('action', url);
         $('#item-delete').text(jabatan.nama);
      }
   </script>
@endpush
